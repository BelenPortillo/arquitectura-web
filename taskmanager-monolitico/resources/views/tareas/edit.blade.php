    ...
    <div class="bg-white ring-1 ring-gray-200 shadow rounded-lg p-6 space-y-4">
      ...
      <form action="{{ route('tareas.update', $tarea) }}" method="POST" class="space-y-4">
        ...
        <div>
          ...
          <input type="text" name="titulo" id="titulo" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                 value="{{ old('titulo', $tarea->titulo) }}" required>
        </div>

        <div>
          <label for="descripcion" class="block text-sm font-medium text-gray-700">Descripción</label>
          <textarea name="descripcion" id="descripcion" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('descripcion', $tarea->descripcion) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input type="date" name="fecha_fin" id="fecha_fin" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                   value="{{ old('fecha_fin', $tarea->fecha_fin) }}">
          </div>
          <div>
        ...
        <div>
          ...
          <select name="user_id" id="user_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
            @foreach($usuarios as $u)
              <option value="{{ $u->id }}" {{ $tarea->user_id == $u->id ? 'selected' : '' }}>
                {{ $u->name }} ({{ $u->email }})
              </option>
            @endforeach
          </select>
